...
class Album extends Gui{
    ...
    async setNextImgs(nextToken){
        ...
        if(! res){
            console.log(new Error('snAcKc batch failed'));
            return null;
        }
        res = res.split(/\n\d+\n/)[1];
    ...
    async parseAlbum(){  // Load full album metadata
        try{
            ...
            // New format: <script class="ds:1" nonce="xxx">AF_initDataCallback({key: 'ds:1', hash: '6', data:[...
            const script_p = /<script[^>]*class="ds:\d+"[^>]*>AF_initDataCallback\([^<]+<\/script>/g;
            const script_srcs = src.match(script_p);
            if (!script_srcs || script_srcs.length === 0) {
                throw new Error('Album data script not found; Google Photos page format may have changed');
            }
            ...
            // Extract parameters from AF_initDataCallback({...})
            script_src = script_src.replace(/<script[^>]*>AF_initDataCallback\(/, '');
            script_src = script_src.replace(/\);?\s*<\/script>$/, '');
            // console.log(script_src);
            // Extract the data section from {key: 'ds:1', hash: '6', data:[...}
            script_src = script_src.split(/data:\s*/)[1].split(/,\s*sideChannel:\s*/)[0].replace(/\}$/, '');
            // console.log(script_src);
